...
<body class="bg-blue-50 min-h-screen flex flex-col items-center justify-center p-4">
    ...
    <script>
        const LIFF_ID = "2008573640-Z1aN5Eyn"; 
        let lineUserId = ""; 
        ...
        async function main() {
            try {
                ...
                alert("LIFF Init Failed: " + err.message + "\n(กรุณาตรวจสอบ HTTPS และ Endpoint URL)");
                document.getElementById('loading').classList.add('hidden');
            }
        }
        main();
        ...
        async function handleRegister(e) {
            ...
            const role = document.querySelector('input[name="role"]:checked').value;
            const eduId = document.getElementById('studentId').value; // ใช้ค่าจากช่องนี้ (edu_id)
            ...
            try {
                const res = await axios.post('../api/register.php', {
                    ...
                    edu_id: eduId, // ส่งเป็น edu_id ตามที่แก้ API ไปก่อนหน้า
